...
class Crud_model extends CI_Model {
	...
	function get_students_roll_sort($class_id,$section_id="") {
		if(empty($section_id))
    	    $query = $this->db->order_by("roll","ASC")->get_where('student', array('class_id' => $class_id));
		else
			$query = $this->db->order_by("roll","ASC")->get_where('student', array('class_id' => $class_id,'section_id' => $section_id));
	...
	function get_point_to_grade($point_obtained) {
	
		$query = $this->db->query('SELECT a.name FROM grade a, grade b WHERE a.grade_point <='.$point_obtained.'  AND b.grade_point >= '.$point_obtained.' LIMIT 1');
